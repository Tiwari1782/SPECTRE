                  ...
                  <div className="match-action-buttons">
                    ...
                    >
                      {loadingExplanation[match.user._id] ? (
                        ...
                        <><span className="material-symbols-outlined" style={{ fontSize: '1rem', marginRight: '4px' }}>visibility</span> Explained</>
                      ) : (
                        <><i className="fa-solid fa-brain" style={{ marginRight: '4px' }}></i> AI Explain</>
                      )}
                    </button>
                    <button className="btn btn-primary btn-sm flex-center" onClick={() => window.location.href = `/chat?target=${match.user._id}`}>
                      <i className="fa-solid fa-message" style={{ marginRight: '4px' }}></i> Chat
                    </button>
                  </div>
                </div>
              </div>
                   ...
                   <div className="match-breakdown-grid">
                      {Object.entries(match.breakdown)
                          ...
                          .map(([key, value]) => {
                             ...
                             return (
                               <div key={key} className="breakdown-bar-container">
                                 <div className="breakdown-bar-labels">
                                    <span className="breakdown-bar-title">{labels[key] || key}</span>
                                    <span className="breakdown-bar-value">{value}</span>
                                 </div>
                                 <div className="breakdown-bar-track">
                                    <div className="breakdown-bar-fill" style={{ width: `${(value/30)*100}%`, background: colors[key] || '#6366f1' }} />
                                 </div>
                               </div>
                             );
                          })}
